...
devfifo_read(struct Fd *fd, void *buf, size_t n)
{
	...
	while(1){
		...
		fsipcbuf.read_fifo.req_n = n - res;

		if ((r = fsipc_fifo(FSREQ_READ_FIFO, NULL)) < 0){
			if (r == -E_FIFO){
				memmove(buf + res, fsipcbuf.readRet.ret_buf, fsipcbuf.readRet.ret_n);
				res += fsipcbuf.readRet.ret_n;
				if (debug)
					cprintf("read wait\n");
				sys_yield();
				continue;
		...
		memmove(buf + res, fsipcbuf.readRet.ret_buf, r);
		res += r;
		break;
	}
...
devfifo_write(struct Fd *fd, const void *buf, size_t n)
{
	...
	while(1){
		...
		fsipcbuf.write_fifo.req_n = n - res;
		memmove(fsipcbuf.write_fifo.req_buf, buf + res, n - res);

		if ((r = fsipc_fifo(FSREQ_WRITE_FIFO, NULL)) < 0){
			if (r == -E_FIFO){
				res += fsipcbuf.writeRet.ret_n;
				if (debug)
					cprintf("write \n");
				sys_yield();
				continue;
